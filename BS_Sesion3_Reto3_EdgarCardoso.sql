USE tienda;
-- 1
CREATE VIEW puestos_169 AS 
SELECT CONCAT(empleado.nombre, ' ',empleado.apellido_paterno) AS nombre, puesto.nombre
AS puesto FROM empleado 
LEFT JOIN puesto ON empleado.id_puesto = puesto.id_puesto;

-- 2
CREATE VIEW empleado_articulos_169 AS
SELECT sub1.clave AS clave_venta, CONCAT(empleado.nombre, ' ', empleado.apellido_paterno)
AS nombre_empleado, 
...
-- 3
CREATE VIEW numero_ventas_169 AS
